...
function VerifyEmail() {
  ...
  const handleChange = (index, value) => {
    ...
    if (value.length > 1) {
      ...
      const pastedCode = value.slice(0, 6 - index).split(""); // Limit paste to available slots
      pastedCode.forEach((digit, i) => {
        newCode[index + i] = digit;
      });
      setCode(newCode);

      // Set focus to the next available empty field
      const nextIndex = index + pastedCode.length;
      if (nextIndex < 6) {
        inputRefs.current[nextIndex].focus();
      }
    } else {
      // Handle single digit input
      newCode[index] = value.replace(/\D/, ""); // Allow only numeric input
      setCode(newCode);
